class Tabela:
    def __init__(self, tamanhoMax=100):
    ...
    def buscar(self, chave):
        ...
        #return self.buscaBinariaNormal(chave)
        return self.buscaLinearRecursiva(chave, self.inicio)
        #return self.buscaBinariaRecursiva(chave, self.inicio, self.fim)

    #função de busca linear recursiva
    def buscaLinearRecursiva(self, chave, posicao):
        if self.vazia() or (self.chave[posicao] != chave and posicao >= self.fim):
            return 0
        if self.chave[posicao] == chave:
            return posicao
        else:
            return self.buscaLinearRecursiva(chave, posicao+1)

    #função de busca binária recursiva
    def buscaBinariaRecursiva(self, chave, inicio, fim):
        if self.vazia() or (inicio >= fim and self.chave[inicio] != chave):
            return 0
        meio = (inicio+fim)//2
        if self.chave[meio] == chave:
            return meio
        elif self.chave[meio] > chave:
            return self.buscaBinariaRecursiva(chave, inicio, meio-1)
        elif self.chave[meio] < chave:
            return self.buscaBinariaRecursiva(chave, meio+1, fim)

    def buscaBinariaNormal(self, chave):
        inicio = self.inicio
        fim = self.fim
        meio = 0
        if self.vazia():
